...
const extractOnly = (date, wishlist, params = {}) => {
  ...
  wishlist.forEach(option => {
    ...
    if (option === "month" && !monthAsNumber) foundDate = months[foundDate];
...
const transformSecondsToDate = (seconds, params = {}) => {
  ...
  if (!dontSetMidnight)
    transformedDate = moment(transformedDate)
      .startOf("day")
      .toDate();
...
const lastAndNextMonth = date => ({
  endOfLastMonth: moment(date)
    .subtract(1, "months")
    .endOf("months")
    .toDate(),
  startOfNextMonth: moment(date)
    .add(1, "months")
    .startOf("months")
    .toDate()
});
