...
const additionalBalances = () =>
  firebase.firestore().collection("additional_balances");
...
const calculate = async ({ userUid, spendingDate }) => {
  try {
    const lastMonthSpendingDate = moment(spendingDate)
      .subtract(1, "months")
      .toDate();

    const { monthlyIncome } = await userService.get(userUid);
    const currentExpenses = await expensesServices.getAll(
      userUid,
      spendingDate
    );
    const userBalanceHistory = await getHistoryByDate({
      userUid,
      spendingDate: lastMonthSpendingDate
    });
    const userAdditionalBalances = await calculateAdditionalBalancesOnly({
      userUid,
      spendingDate
    });

    const lastMonthBalance =
      userBalanceHistory.balance !== undefined
        ? _treatFloatNumber(userBalanceHistory.balance)
        : 0;

    let remainingBalance = _treatFloatNumber(
      monthlyIncome + lastMonthBalance + userAdditionalBalances
    );

    // Calculate remaining balance for this month
    currentExpenses.forEach(
      ({ amount, differenceAmount = 0 }) =>
...
const calculateAdditionalBalancesOnly = async ({ userUid, spendingDate }) => {
  try {
    const userAdditionalBalances = await getAdditionalBalances({
      userUid,
      spendingDate
    });
...
const recordHistory = async ({ userUid, spendingDate }) => {
  try {
    const subtractedSpendingDate = moment(spendingDate)
      .subtract(1, "months")
      .toDate();

    const { monthlyIncome } = await userService.get(userUid);
    const balance = await calculate({ userUid, spendingDate });
    const lastMonthBalance = await getHistoryByDate({
      userUid,
      spendingDate: subtractedSpendingDate
    });
...
const getAllHistory = async userUid => {
  try {
    const allHistory = await balanceHistory()
      .where("user", "==", userUid)
      .get();
    return allHistory.docs.map(history => ({
      id: history.id,
      ...history.data()
    }));
  } catch (err) {
...
const getAdditionalBalances = async ({ userUid, spendingDate = null }) => {
  try {
    let balances = additionalBalances()
      .where("user", "==", userUid)
    ...
    return balances.docs.map(balance => ({
      id: balance.id,
      ...balance.data()
    }));
  } catch (err) {
...
const reset = async userUid => {
  try {
    ...
    balancesToDelete.forEach(({ id }) =>
      batch.delete(additionalBalances().doc(id))
    );
    balanceHistoryToDelete.forEach(({ id }) =>
      batch.delete(balanceHistory().doc(id))
    );
