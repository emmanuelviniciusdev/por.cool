...
const changePassword = async (currentPassword, newPassword) => {
  try {
    ...
    return _response({
      error: true,
      message: "ocorreu um erro ao alterar a senha [1]"
    });
  }
};
...
const changeEmail = async (password, newEmail) => {
  try {
    ...
    if (newEmail === user.email)
      return _response({
        error: true,
        message: "então quer dizer que seu novo e-mail é igual ao atual? 😞"
      });
    ...
    return _response({
      error: true,
      message: "ocorreu um erro ao alterar o seu e-mail [1]"
    });
  }
};
