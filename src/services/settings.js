...
const messages = {
  maintenance:
    "Uma pequena manutenção de rotina está sendo realizada neste exato momento. Por favor, tente novamente mais tarde.",
  blockUserRegistration:
    "o registro de novos usuários encontra-se temporariamente indisponível",
  unexpectedError:
...
const checkMaintenances = async (filterBy = "") => {
  ...
  try {
    ...
    Object.keys(settings).map(setting => {
      status[setting] = settings[setting]
        ? messages[setting]
          ? messages[setting]
          : messages.unexpectedError
        : false;
