...
const getAll = async (userUid, validity = null) => {
  ...
  try {
    let allExpenses = expenses().where("user", "==", userUid);
 ...
 * @returns boolean
 */
const finishCurrentSpendingDate = async (
  userUid,
  currentLookingAtSpendingDate,
  params = {}
) => {
  const { autoClone } = params;

  try {
    const nextLookingAtSpendingDate = moment(currentLookingAtSpendingDate)
      .set("date", 1)
      .add(1, "months")
      .startOf("day")
      .toDate();
    const expensesToClone = await _getExpensesToClone(
      userUid,
      currentLookingAtSpendingDate,
      nextLookingAtSpendingDate
    );
    ...
    await userService.update(userUid, {
      lookingAtSpendingDate: nextLookingAtSpendingDate
    });
 ...
 * @returns Promise
 */
const _getExpensesToClone = async (
  userUid,
  currentLookingAtSpendingDate,
  nextLookingAtSpendingDate
) => {
  try {
    ...
    const expensesWithValidity = await expensesToClone
      .where("validity", ">=", nextLookingAtSpendingDate)
      .get();
    const expensesWithNoValidity = await expensesToClone
      .where("validity", "==", null)
      .where("indeterminateValidity", "==", true)
      .get();
    ...
    return [
      ...expensesWithValidity.docs.map(expense =>
        mapUpdateExpenseDates(expense.data())
      ),
      ...expensesWithNoValidity.docs.map(expense =>
        mapUpdateExpenseDates(expense.data())
      )
    ];
  } catch (err) {
...
const getSpendingDatesList = async ({ userUid, lookingAtSpendingDate }) => {
  ...
  const currentDate = dateAndTimeHelper.extractOnly(lookingAtSpendingDate, [
    "year",
    "month"
  ]);
  ...
  allExpenses.forEach(({ spendingDate }) => {
    ...
    if (!trash.includes(year)) {
      spendingDatesList.push({
        year,
        months:
          year === currentDate.year
            ? months.slice(0, months.indexOf(currentDate.month) + 1)
            : months
      });
      trash.push(year);
    }
