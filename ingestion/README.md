...
The service can optionally update Firestore settings documents with sync metadata after successful ingestion.
...
| `FIREBASE_SYNC_METADATA_SERVICE_NAME` | Service name for sync metadata entries | `porcool-ingestion-non-relational-db-to-relational-db` |
...
#### Sync Metadata Behavior

When enabled, the service upserts an entry in the `syncMetadata` array of all documents in the Firestore `settings` collection:

```json
{
  "name": "porcool-ingestion-non-relational-db-to-relational-db",
  "latestSyncDatetime": "2024-01-15T10:30:00Z"
}
```
